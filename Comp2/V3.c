...
task update_dist(){
	while (true){
		insertDistBuffer(avg_dist);
		sleep(10);
		avg_dist=average();

	}
}
void resetDistBuffer(){
	stopTask(update_dist);
	int i;
	for(i=0;i<bufferSize-1;i++){
		distBuffer[i]=255;
	}
...
task findMinimumDistance(){
	minDist = avg_dist;//must be global variables
...
void moveToDistance(int distFromWall, int power){//the use of this function is to get the robot to a certain distance away from the wall
	...
	while(avg_dist>distFromWall){
		error = abs(avg_dist - distFromWall)*gain;
		if(error<power){
			if(error<minPower){
				error=minPower;
			}
...
task main(){
	...
	moveToDistance(20, motorPower); //moves to within 15mm of the opposite wall
